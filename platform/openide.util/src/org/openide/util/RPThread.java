 ...
 */
final class RPThread extends Thread {
     ...
     */
    void attachTo(RequestProcessor src) {
        synchronized (lock) {
            //assert(source == null);
            source = src;
            lock.notify();
     ...
     */
    final void doEvaluate(RequestProcessor.Task t, Object processorLock, RequestProcessor src) {
    ...
    void interruptTask(RequestProcessor.Task t, RequestProcessor src) {
        if (t != todo) {
    ...
    boolean interrupt(RequestProcessor.Task t, RequestProcessor src) {
        if (t != todo) {
            return false;
        }
        interrupt();
